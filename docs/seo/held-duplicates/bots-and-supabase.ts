...
The first database bot query returned a plausible activation count. Weeks
later, someone checking the Supabase source found that the query had
...
That is one failure that matters in database bot access, but it is not the
...
Everything below makes wrong reads visible and writes mechanically unavailable.
The database identity must be unable to insert, update, delete, run migrations,
change policies, invoke mutating functions, or administer the project.

## Keep Supabase read-only: A metric definition is a written artifact or it is nothing
...
## Keep Supabase read-only: Write one registry entry all the way through before you write a second
...
## Keep Supabase read-only: Know the definition trap sitting under each common metric
...
## Keep Supabase read-only: Require the query, the source, and the timestamp on every row
...
## Keep Supabase read-only: Keep the registry where editing is easy and auditing is possible
...
## Keep Supabase read-only: Put what it could not compute in the report, every time
...
## Keep Supabase read-only: Paste this charter and change only the definitions file name

\`\`\`text
You are my Read-Only Database Analyst.
...
// READ ONLY
Use only the approved Supabase reporting identity and allowlisted SELECT
queries. Never insert, update, upsert, delete, truncate, copy, call a
mutating function, run DDL, run a migration, change a policy, reveal a
secret, or administer the project. Never work around a denied statement.
You never change metrics.md. Quote suspected definition errors in
DEFINITION FLAGS and leave both the file and database untouched.
...
## Keep Supabase read-only: Keep it read-only against every system it touches
...
## Keep Supabase read-only: Expect definition drift, and make it loud rather than impossible
...
## Keep Supabase read-only: Match the symptom to the failure sitting underneath it
...
## Keep Supabase read-only: Watch what changes between week one and week thirty
...
## Keep Supabase read-only: Recompute one metric by hand every week, chosen at random
...
## Keep Supabase read-only: Answer the objection that your BI tool already does this
...
## Keep Supabase read-only: Grow from three metrics to a weekly pack, in this order
...
Keep the historical result series outside the bot runtime. Preserve the exact
query, definition version, row count, and execution time so you can reproduce a
report without trusting the generated explanation.
...
### How should database bot access be made read-only?

Enforce read-only database bot access in the database identity, not only in the
prompt. Give the bot a dedicated reporting credential that can run only the
approved SELECT workload against the required schema or views. Withhold insert,
update, delete, DDL, migration, policy, function, secret, and administrative
paths. Test the restriction with harmless denied statements before production.
The charter should repeat the same boundary and require the exact query, source,
scope, row count, and timestamp in every report, but the credential remains the
control that makes a mistaken instruction fail.

### What should a Supabase bot do when a query fails?

A Supabase bot should report the failed query, sanitized error class, attempted
scope, and timestamp, then stop. It should not broaden the query, switch to a
more privileged credential, inspect unrelated schemas, guess a value, or retry
a statement that the database denied. A timeout, access denial, and empty result
mean different things, so preserve that distinction in the report. If a query
returns unexpectedly few rows, label the output incomplete until a human checks
the source. Failure should reduce confidence and authority, never trigger an
automatic search for a more powerful route.

### Should a database bot fix bad rows that it finds?

No. A bot that edits the records it also reports on destroys the clean evidence
you need to evaluate its findings. Let it identify a stable record key, the
suspected issue, the query that exposed it, and a proposed correction in a
private report. A human or a separately governed migration process owns the
change. This boundary also covers apparently harmless cleanup, deduplication,
backfills, and null replacement. Each can affect application behavior, policies,
triggers, analytics, or downstream jobs. Read-only keeps a questionable finding
cheap to reject and preserves the original state for investigation.

### Can a Supabase reporting bot use a production database?

It can read production only when production is genuinely required, the database
enforces a dedicated read-only identity, the query scope is bounded, and the
workload has been tested against a safe copy first. Prefer approved views that
exclude sensitive columns and expose only the rows needed for the report. Set
query limits and timeouts through the connection path you control, then inspect
database logs after early runs. If the job can use a replica, export, or sanitized
snapshot, choose that instead. Convenience alone is not a reason to place a bot
