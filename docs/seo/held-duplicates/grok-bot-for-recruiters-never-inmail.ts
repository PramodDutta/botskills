...
export const post: BlogPost = {
  title: 'Grok Bot for Recruiters: Score Saved Pages, Never InMail',
  description: 'Use grok bot for recruiters to score saved candidate pages with evidence, preserve recruiter judgment, and keep every InMail behind human review.',
  date: '2026-08-29',
  category: 'Guide',
  content: `
# Grok Bot for Recruiters: Score Saved Pages, Never InMail

Nia does not need a bot that looks busy. Nia needs a saved-page shortlist for a staff data engineer to arrive with enough evidence to review and no hidden external action. The useful setup is narrow: collect named inputs, preserve uncertainty, produce a private artifact, and stop at the boundary.

The boundary for this workflow is exact: **Never open, draft, queue, or send InMail. Score only pages a recruiter has already saved.** That sentence is the control surface. A friendly bot name, a routine label, and a successful sign-in do not replace it. The longer explanations live in the [inheritance guide](/blog/what-a-pasted-prompt-inherits), [boundary guide](/blog/how-to-write-a-boundary-line), [approval guide](/blog/what-an-approval-actually-governs), [permission guide](/blog/a-boundary-is-not-a-permission), [architecture guide](/blog/screens-are-not-boundaries), [operator guide](/blog/learn-grok-bot). This page spends its time on the failure you searched for.

On Thursday, the source page omitted a location and the bot treated the blank as remote eligibility. The shortlist looked cleaner than the evidence. Nia caught it because the output carried the source field beside the score.

Start from one of these real catalog patterns when it matches the work: [starting workflow](/bots/lead-scout), [evidence helper](/bots/recruiting-reviewer), [review helper](/bots/prospecting-sheet-builder), [briefing helper](/bots/source-verifier). Each is a starting charter, not an authorization grant. Replace its sample sources, owner, review window, and output path before the first live run.
...
## Make the first decision from the artifact, not the bot name for saved-page recruiting
...
## Write the irreversible line before connecting the source for saved-page recruiting
...
## Separate evidence collection from the decision it informs for saved-page recruiting
...
## Preserve unknown as an honest output state for saved-page recruiting
...
## Rank sources by authority before comparing timestamps for saved-page recruiting
...
## Paste a charter that can stop without improvising for saved-page recruiting
...
## Walk one named operator through the first complete run for saved-page recruiting
...
## Trace the specific failure back to the missing rule for saved-page recruiting
...
## Route every exception to one accountable person for saved-page recruiting
...
## Test the boundary with a fixture designed to cross it for saved-page recruiting
...
## Answer the strongest case for granting more autonomy for saved-page recruiting
...
## Verify the result with a check that is allowed to fail for saved-page recruiting
...
## Name the adjacent case this page does not cover for saved-page recruiting
...
## Reconcile Nia's case with a four-state ledger before the next run for saved-page recruiting
...
|---|---|---|
| Saved page has the required evidence | Score and cite the saved page | Proceed and retain evidence |
| Evidence is missing or ambiguous | Return unknown and request review | Stop automatic progress |
| Candidate looks strong but was not saved | Ignore the page until a recruiter saves it | Escalate with the gap named |
| A message would improve response speed | Stop and hand the candidate to the recruiter | Require explicit human handling |

For Nia, the first pass is intentionally manual. Number the units from 1 through 8, an arbitrary rehearsal size, and attach the source URL or record identifier to each row. Add observed-at time, output path, completion evidence, and reviewer. Eight is not a product limit. It is small enough to compare every row without sampling.

The walked failure matters because the tempting repair is the wrong repair. On Thursday, the source page omitted a location and the bot treated the blank as remote eligibility. The shortlist looked cleaner than the evidence. Nia caught it because the output carried the source field beside the score. The bot should not smooth that gap into a confident sentence. It should state which step completed, which step did not, and which step has an unknown state. That output gives the reviewer something actionable without pretending the missing evidence exists.

Use this charter fragment as the fixed rule for the next rehearsal:

\`\`\`text
OBJECTIVE
Prepare a saved-page shortlist for a staff data engineer.
...
BOUNDARY
Never open, draft, queue, or send InMail. Score only pages a recruiter has already saved.
