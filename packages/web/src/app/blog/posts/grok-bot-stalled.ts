...
export const post: BlogPost = {
  title: 'Grok Bot Stalled Mid-Job: How to Restart Without Doubling the Work',
  description:
    'Grok Bot stalled mid-job is usually a wait, a selector, or a 2FA wall. Inspect artifacts, then restart without duplicating the half-finished writes.',
  date: '2026-08-27',
  category: 'Reference',
  content: `
# Grok Bot Stalled Mid-Job: How to Restart Without Doubling the Work

The last chat line still says the bot is working, the Agent Computer preview looks like a still photo, and there is no error you can copy. That is a grok bot stalled mid-job: a silent sit, not a crash. Most people type "start over" because sitting feels like failure. Start over is how a half-finished write becomes two writes.

The usual causes are a wait on an approval you did not see, a page whose buttons moved, a 2FA wall, or a tool call that ended without a result. None of those wipe the files under \`/workspace\`. Inspect, name the last checkpoint, resume from the next one, and never re-run a send. A separate troubleshooting catalog lists fifteen failures, including a frozen run. This page is the restart protocol for that one row.

There is no published stall timeout number, so this article will not invent one. Do not wait for an automatic kill you have not been promised. Open the screen. Read the disk. Then type a resume that names a checkpoint.

## Read a silent sit as a wait until the screen says otherwise
...
## Open the bot's screen before you type any restart
...
## Resume from the last written checkpoint, never from step one
...
## Refuse to re-run any send that might already have left
...
## Reconstruct a half-finished bug hunt from the screenshots already saved
...
## Put numbered checkpoints in the charter so a stall has a landing
...
## Keep this page off the fifteen-failure troubleshooting hub
...
## Answer the objection that wiping and starting over is cleaner
...
## Prove the resume with a check that fails if you doubled the work
...
## Hand the remaining step to a human when the wall is a login
...
## Park irreversible verbs behind approval so a stall cannot send twice
...
## Trust leftover files on the shared computer as the source of truth
