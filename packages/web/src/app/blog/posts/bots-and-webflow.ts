...
export const post: BlogPost = {
  title: 'Bots and Your CMS: Draft Only, Always',
  description:
    'Bots and Webflow (or any CMS) draft in a private folder. They never hit Publish. Confirm Webflow on the vendor page. A CMS login on the shared computer is a house key.',
  date: '2026-08-28',
  category: 'Guide',
  content: `
...
Primer: [what a Grok Bot is](/blog/what-is-a-grok-bot). The calendar cousin is [Content Planner Manager](/bots/content-planner-manager), which never publishes a Webflow item. The help-center cousin is [Help Center Updater](/bots/help-center-updater). The recycle cousin is [Evergreen Content Flywheel](/bots/evergreen-content-flywheel), which never schedules a recycled post. This page is the marketing site: three diffs, one human Publish.

## Treat a CMS Designer login as a house key on the shared computer

Grok Bot does not give each bot a private machine. All bots on an eligible account share one persistent cloud computer assigned to the user, not to a bot ([computer and apps](https://docs.x.ai/grok-bot/computer-and-apps)). Each bot gets a screen. Screens are work surfaces, not vaults. The docs are blunt: do not use separate Bots as a security boundary ([approvals, security and privacy](https://docs.x.ai/grok-bot/approvals-security-and-privacy)).
...
The bot does not pick the winner. Ranking copy is not a publish grant. [Inbox Triage](/bots/inbox-triage) drafts and never sends. This desk drafts and never publishes. The parallel is the stop, not the artifact.
...
If the real request is "the bot should operate the Designer because pasting is slow," the answer is no. Slow is the review. [Building a bot that drafts but never sends](/blog/bot-that-never-sends) is the same trade in mail. Speed without a stop is how a comparison table ships a dead company.
...
[Help Center Updater](/bots/help-center-updater) uses the same current-text-beside-proposed-text shape. Steal the shape. Do not steal a publish grant. Claude Code, SKILL.md, and CLAUDE.md compatibility is Grok Build, never Grok Bot. Do not drop a SKILL.md into this folder and expect Grok Bot to obey it.
...
| Live page changed and no folder file exists for that URL | A sibling bot used the shared cookie, or a human published from the same seat | Inventory other bots. Screens are not isolation. See [one computer, many screens](/blog/grok-bot-one-computer-many-screens) |
...
The landing-page drafter is not the only resident. [Lead Scout](/bots/lead-scout) and [Chief of Staff Briefing](/bots/chief-of-staff-briefing) share the computer if they sit on the same eligible account. A briefing bot that "checks the website" will use the Designer cookie if it is there. Sign out is part of the job.
...
Least privilege still applies: [least privilege bots](/blog/least-privilege-bots). If a connector offers a publish scope, do not grant it. Public pages are public. Logged-out reads are enough for a diff.

Do not use a second named bot as a sandbox for the CMS. That is the myth [do not use separate Grok Bots as a security boundary](/blog/grok-bot-not-a-sandbox) exists to kill. Two names, one computer, one cookie jar. See [shared computer security](/blog/grok-bot-shared-computer-security). Two eligible users are two computers. One user with ten bots is still one computer.
...
This page also stops when the job is a help article. Use [how to keep a help center current automatically](/blog/how-to-automate-help-center-updates). The updater proposes current text beside proposed text and never publishes.

This page stops when the job is a directory listing. Use [research a directory page a day without publishing it](/blog/grok-bot-directory-business). Submit on a live index is a different button. Same stop.

This page stops when the job is YouTube listing copy. Use [a YouTube manager that drafts and never publishes](/blog/grok-bot-youtube-manager). Keep using this page while the job is three marketing landing page diffs, Webflow or any visual CMS confirmed on the vendor page, and a human who publishes at most one of them.
...
**Keep reading:** [One computer, many screens](/blog/grok-bot-one-computer-many-screens), [keep a help center current automatically](/blog/how-to-automate-help-center-updates), [research a directory page without publishing it](/blog/grok-bot-directory-business).
