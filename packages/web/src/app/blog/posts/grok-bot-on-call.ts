...
export const post: BlogPost = {
  ...
  description:
    'Grok Bot on call is a named human with a desktop, not iPhone edit. There is no audit log outside Enterprise. Pause on iPhone. Diagnose at a desk. Name the owner first.',
  date: '2026-08-27',
  category: 'Guide',
  content: `
# Who Is On Call When a Grok Bot Misfires at 3am

Nobody in the product is on call. A grok bot on call is a person you wrote down, sitting near a Mac or a Windows desk, and at 03:12 the person who got pinged is in bed with an iPhone. Pause is the most useful verb that phone has. Editing and testing a routine still need the desktop app; the phone can now show run history and delete a routine. There is no audit view of bot actions outside Enterprise, so the app will not tell you whether the customer mail already left. Write the owner into the charter before that night, because Grok Bot will not name them for you.

This page is staffing. It is not a restart protocol. If the chat went silent with files half written, that is [Grok Bot stalled mid-job](/blog/grok-bot-stalled). If a 09:00 DM never arrived because the owner bot was deleted, that is [a routine that did not run](/blog/grok-bot-routine-did-not-run). Stay here when a send almost left, or did leave, and you need to know who gets paged, what they can do from a phone (pause first), and why delete is not the first move.
...
Name a person who can reach a supported desk. Supported clients are macOS, Windows and Linux on desktop, plus iPhone (iOS 18 or later) and Android (9 or later) companion apps; the iOS app also runs on iPad (iPadOS 18 or later). There are Linux desktop and Android apps as of September 2026. The cloud computer is a managed Linux VM, which is not a laptop you can SSH into from bed. A Linux-only engineer can pause from a phone and diagnose on the Linux desktop app.
...
Page the person who can sit down. The incident is a proposed action on a shared cloud computer. The only honest view is the bot's screen plus the files under \`/workspace\`. A founder in a taxi with an iPhone can freeze the next attempt and open history. They cannot read the proposed body the way a desk can, or patch the charter.
...
|---|---|---|---|
| Named owner with a Mac, Windows or Linux laptop in the house | Yes, if they have iOS 18+ | Yes | Yes, if they agreed in writing |
...
| Android-only contractor | Yes, from the Android app | No, not without a desktop | Only for pause, if they agreed in writing |
...
The 3am instinct is to tap through the app until the problem goes away. Documented mobile scope for routines is pause and resume, run history, and delete. Editing and testing a routine still need the desktop app. Teach by demonstration is unavailable on iPhone. You will not rewrite the discount or fire a one-shot test from the lock screen, and you should not delete the bot from it.
...
iPhone on iOS 18+ is the companion. Android is, since September 2026. iPad runs the iOS app on iPadOS 18 or later. If you have no supported phone, you have no remote freeze. Pause the send-capable bots before you sleep in a place with no supported desktop and no supported phone.
...
The 3am delete also destroys the evidence you need at 07:00. The twenty most recent run records per routine die with the routine. The routine dies with the bot. The draft file and the mailbox session remain. You wake up to a missing sidebar card, a customer who may already have the mail, and a fossil markdown file that looks like the job is still alive. The phone can delete now, which is exactly why the charter says it must not. [How to delete a Grok Bot without leaving logins behind](/blog/delete-a-grok-bot-safely) is teardown for a planned retirement. A misfire is not a retirement.
...
| Missed routine | Empty 09:00, or a bot named temp missing from the sidebar | Ask which bot owned the routine, from any client that can open history | Treating it as a 3am send incident |
...
Grok Bot has no audit view of bot actions outside Enterprise. Twenty run records per routine are a sliding window, not a ledger. Chat is a commentary track. The destination is the log that counts: Sent, the ticket, the CRM, the published URL, the bank. [Grok Bot has no audit view outside Enterprise](/blog/grok-bot-no-audit-log-yet) is how you keep receipts on purpose. On-call is how you use them when the pager fires.
...
You cannot finish this inventory from iPhone. The phone reads run history, but the inventory is files and Sent on the computer, and that is a desk job. Pause, then inventory at the named desk. A timeline from memory is how you apologize for mail that never left, or miss the one that did.
...
Do not hand the Cursor login around as the on-call token. None of the SKUs that include Grok Bot ship a team on-call calendar for bots. You still write the name. Cursor Pro at $20 a month is the cheapest paid path that includes it. Cursor Hobby, the free plan, does not. An individual SuperGrok subscription can be linked instead. Every member of a self-serve Cursor Teams plan has it.
...
| Phone drill | Pause and history work. Edit and test do not | You changed a line from iOS, or you resumed from bed |
...
Approve-from-bed wins only when three things are true at once: you can read the full proposed body, you would send that exact body yourself, and you have already checked that the same mail is not already in Sent. The iPhone app is not built for that triple. You can read run history but not edit the routine. You cannot list \`/workspace\`. You cannot patch the charter if the body is wrong. You can pause. Pause is the honest pocket verb.

The cost of waiting until morning, if send is parked, is a delayed draft. The cost of a 3am yes on a 40 percent discount is a customer who now believes the number, plus a thread you cannot pull back. There is no audit view outside Enterprise to reconstruct what you tapped. Your receipt is whatever you write down after.

If the founder travels without a supported laptop, they are not on-call for diagnosis that week. Pause the send-capable bots before the flight, or take a supported laptop. Linux in a hotel is a Grok Bot client since September 2026. iPad runs the iOS app, which is a phone client, not a desk. The [safety checklist](/blog/grok-bot-safety-checklist) is the pre-flight for connecting mail. This page is the pre-flight for sleeping while mail is connected. Write the pillow rule into the charter so 03:12 Priya, who is also the founder, does not renegotiate it half asleep.
...
A Linux-only household can diagnose on the Linux desktop app, which the docs list as of September 2026. Staffing still cannot invent a client for a household with no desktop at all.
...
Companies that need a vendor audit log, a team on-call calendar, and a dedicated VM per bot are describing a product Grok Bot does not ship today. Separate bots are not a security boundary. An audit view of bot actions does not exist outside Enterprise. Write the gap down. Do not staff as if it were filled.

**Keep reading:** [Grok Bot iPhone limits: what still needs desktop](/blog/grok-bot-iphone-cannot-edit), [Grok Bot has no audit view outside Enterprise](/blog/grok-bot-no-audit-log-yet), [How to set Grok Bot approvals so sends never slip through](/blog/how-to-set-grok-bot-approvals).
...
You can pause. That is the documented pocket pair, with resume, and resume is the wrong 3am verb. Editing and testing a routine still need the desktop app; the phone can now show run history and delete a routine. You will not rewrite a discount from the lock screen, and you should not delete the bot from it. Pause so the job stops trying. Get to a Mac, Windows or Linux desk. Read the proposed action. Check Sent. Deny the send if it is still sitting. There are Linux desktop and Android apps as of September 2026, so a phone-plus-Linux bag can now install both: a Linux desktop app and an Android or iPhone app.

### Should I delete the bot when it almost sends at 3am?

No. Deleting the bot removes its profile, conversation, and routines. It does not unsend mail, sign out of Gmail, or erase files on the shared computer. It also destroys the twenty run records you may want in the morning. The phone can delete now, which is why the charter forbids it. Pause from the phone. Inventory at a desk. Deny the approval if the send is still sitting. Copy anything you need off the machine before you retire the bot on a calm afternoon, and revoke the mailbox session separately.

### How do I know the customer mail already left if my plan has no audit log?

Look at the destination, not the chat, and not a NOTES.md that claims success. Grok Bot has no audit view of bot actions outside Enterprise. Open Sent, the ticket thread, or the CRM activity for that customer. One artifact means it left. Zero means you may still have a parked send to deny. Two means someone doubled it. You cannot finish that check from iPhone history alone, because a run record says a run happened, not what it sent. Pause first, then inspect from the named desk, and write what you found into a file you own.
`,
};
