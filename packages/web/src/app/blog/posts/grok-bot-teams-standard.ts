...
export const post: BlogPost = {
  title: 'Grok Bot on Cursor Teams Standard: The Per-Seat Path',
  description:
    'Every member of a self-serve Cursor Teams plan gets grok bot teams standard access, no Premium seat needed. It is still one computer per person, not a shared bot roster.',
  date: '2026-08-27',
  category: 'Guide',
  content: `
# Grok Bot on Cursor Teams Standard: The Per-Seat Path
...
|---|---|---|
| Cursor Hobby | Not included | Move them onto a seat on the Teams plan, or send them to the trial if they are only sampling |
| Cursor Pro at $20 | Included. Individual door, not this SKU | Stay on Pro if they are not actually on the team |
| Cursor Teams Standard seat | Included. This SKU | Confirm the app shows this entitlement for that person, then install |
| Cursor Teams Premium seat | Included. Same Bot product | You already have a door. Premium is not a better bot |
| Cursor Pro+ at $60 | Included. Individual door, not this SKU | Stay on Pro+ if they are not actually on the team |

A Teams seat is a team SKU, not an individual door. If the company already pays these seats, incremental Grok Bot cost on them is zero. Hobby stays excluded even when the rest of the company is on Standard. Cursor Pro at twenty is an individual door of its own now, and an individual SuperGrok subscription can be linked from the Grok Bot plan screen.

## Treat Teams Premium as the same Bot on a richer Cursor SKU

A Teams Premium seat also includes Grok Bot, but nobody needs one for access: every member of a self-serve Teams plan has it. Confirm seat prices on Cursor's team pricing page the morning you quote them. Premium is a Cursor SKU with a higher price, not a Grok Bot SKU with extra knobs. A higher seat can raise weekly usage; it does not change the product.

After the access check passes, Premium and Standard receive the same documented Bot: one persistent cloud computer per user, a screen per bot, a weekly usage allowance with no published dollar figure, and on-demand overflow billed from model and token cost. There is no Grok Bot-specific spend cap yet, no model picker for members or admins, and no audit view of Bot actions outside Enterprise ([teams and enterprises](https://docs.x.ai/grok-bot/teams-and-enterprises)).
...
| Extra you might think the Premium seat attached to Grok Bot | What either Teams SKU actually attached |
|---|---|
...
| An org-admin audit log of Bot actions | Not on Standard or Premium. Audit logs and Action Recording are Enterprise only |
| Team-level routines you can reassign | A routine assigns a workflow to one Bot. Nothing is team-level |

[How token burn works without a per-bot cap](/blog/grok-bot-spend-cap-and-token-burn) is the operations page for the ceiling neither Teams SKU printed. [Grok Bot has no audit view outside Enterprise](/blog/grok-bot-no-audit-log-yet) is the receipt page. Do not invent an org log to fill a questionnaire.

## Stop treating named bots as company objects you can reassign after a resignation

The expensive story attached to grok bot teams standard is not the seat price. It is the belief that a named bot is an org object you can move to the next hire.
...
Leena runs a six-person content agency that already pays Cursor for the editor. Someone forwards the 21 August widening: Teams Standard includes Grok Bot. Finance signs six seats. Leena tells the team they finally have company bots.
...
|---|---|---|
| Six Teams Standard seats | A company Bot fleet she can staff | Six eligibility stamps, six computers, six people |
...
| People who actually need a Grok Bot computer | Teams seats on the invoice | What you isolated | What you did not buy |
|---|---|---|---|
| 1 (whole agency on Leena) | One seat | Nothing between staff | Convenience, one cookie jar, a fake org roster |
| 6 (one seat each) | Six seats | Six computers | Any console you still paste across accounts, any bot you still try to reassign |
| 6 seats, jobs only on Leena | Six seats | Still one working computer | Five idle stamps and a story about a fleet |

Two user accounts are two computers. Staff who copy a CSV from Leena's machine onto Ravi's have rebuilt a leak by hand. There is no Grok Bot-specific spend cap, but the account-level On-demand monthly limit applies. Overflow after the weekly allowance is on-demand from model and token cost, with no published dollar figure. Bill shape after the door is open is [Grok Bot cost](/blog/grok-bot-cost). If a client console must never sit next to another client's console, you need one eligible account per client, not one named bot. That math lives on [Grok Bot for agencies](/blog/grok-bot-for-agencies-isolation).

## Leave the dummy-team-of-one shave on the shelf if you are actually one person

Teams Standard is not the cheapest paid door for a person of one. Cursor Pro at twenty now includes Grok Bot and is the cheapest paid path, checked 23 September 2026. Teams is a team SKU. You buy it when you already buy Cursor seats.

Do not open a dummy team of one to reach Grok Bot unless you actually want whatever else Cursor attaches to Teams. Confirm those extras on Cursor's current pages. This article will not print a third-party feature list as fact.

If you are one person with no team today, Cursor Pro at twenty is the door, and the Pro+ page is for more weekly usage. If the company already pays for Teams, incremental Grok Bot cost on those seats is zero. Premium is not a better bot. Read [least privilege bots](/blog/least-privilege-bots) before you add a second login. An individual SuperGrok, SuperGrok Plus, SuperGrok Heavy or X Premium+ subscription can be linked instead; their prices live on [x.ai/pricing](https://x.ai/pricing), not on this page. If someone holds both a Cursor plan and a link, Grok Bot uses whichever has more usage. The two never stack.
...
If the FAQ and this page disagree, the FAQ wins. Dates on this page are 25 to 27 August 2026, with plan access rechecked on 23 September 2026. Treat every seat price as something you re-read on the vendor page the morning you pay. Supported clients are macOS (Apple silicon and Intel), Windows (x64 and Arm64), and iPhone on iOS 18 or later. There are Linux desktop and Android apps as of September 2026, and the iOS app also runs on iPad (iPadOS 18 or later). The agent computer is a managed Linux VM, not a Linux desktop app. [Supported platforms](/blog/grok-bot-supported-platforms) is the device grid. Official desktop path: [x.ai/bot](https://x.ai/bot).
...
Grant the purchase. Teams Standard is a team SKU for Cursor. Seat management exists on that side of the house. Grok Bot's access check reads the plan, then hands each eligible user the same Bot product every other eligible plan gets. The computer is assigned to the user account. Routines are per bot. Individual accounts and self-serve Teams still have no audit view of Bot actions; Enterprise has audit logs and Action Recording.

Two admin controls the August teams docs only previewed have shipped. A team admin on Teams Standard can now cap local execution for the whole team. Terminate, which deletes a member's computer while the durable disk is kept, is Enterprise only and for organization admins only, and a halt is not a log. Do not invent an org-admin audit log to cover the gap. Keep your own packet. Keep send on ask. [Grok Bot has no audit view outside Enterprise](/blog/grok-bot-no-audit-log-yet) is how you produce a receipt.
...
An approval controls the proposed action. It does not reverse work already completed. There is still no audit view outside Enterprise, so you keep the ledger yourself. [Approval rules and reversibility](/blog/grok-bot-approval-rules-reversibility) is the control surface after you connect anything that can act. [The safety checklist](/blog/grok-bot-safety-checklist) is the first read before a session lands on the shared machine. Claude Code and SKILL.md compatibility is [Grok Bot versus Grok Build](/blog/grok-bot-vs-grok-build), never this SKU. The Teams seat stays honest when the never-send line stays human, and the owner line stays a person.
...
Yes. Every member of a self-serve Cursor Teams plan has Grok Bot, Standard seat included, with no Premium seat or admin request required, checked on 23 September 2026 against Cursor's Grok Bot plans page. Cursor Hobby, the free plan, does not include it. Every paid Cursor plan does, from Pro at twenty dollars. The include is a per-person stamp, not a shared roster the company owns. Each seat gets one cloud computer assigned to that user, screens that are not security boundaries, no per-Bot spend cap, no model picker, and no audit view of Bot actions outside Enterprise. Confirm the live invoice before you brief staff.
...
No. Self-serve Teams, Standard or Premium, get no audit view of Bot actions. Audit logs and Action Recording are Enterprise only. Twenty run records per routine are a sliding window on one bot, not a company ledger. Pause is a stop, not a log. Admin Terminate, which is Enterprise only, deletes a member's computer and is still not a history of what already ran. Do not write "we have a Teams audit log" into a questionnaire. Keep your own packet, keep send on ask, and treat the missing view as a present-tense product fact. Confirm the teams and enterprises docs the day you fill the form.

### Is Teams Standard cheaper than Pro+ for a solo buyer?

No. Teams Standard is not the cheapest paid door for one person. Teams is a team SKU. Cursor Pro at twenty dollars a month includes Grok Bot and is the cheapest paid path as of 23 September 2026; Pro+ at sixty buys more weekly usage, not a different bot. Open a dummy team of one only if you actually want team billing, seat management, and whatever else Cursor attaches to Teams. If the company already pays for Teams, incremental Grok Bot cost on those seats is zero. Confirm live prices on cursor.com/pricing and Cursor team pricing docs the morning you choose.
`,
};
