...
export const post: BlogPost = {
  title: 'Score Whether This Grok Bot Paid for Itself',
  description:
    'Score whether this grok bot paid for itself with hours you actually saved against overflow you actually saw. 20 run records are not a ledger. No invented allowance.',
  date: '2026-08-28',
  category: 'Guide',
  content: `
...
Log the human hours you actually did not spend. Log overflow you actually saw. Cap the evidence at twenty run records, because that is all the product keeps. Do not invent a weekly allowance in dollars. Do not print a savings percentage as a product fact. There is no audit view of Bot actions yet, so the sheet is the payback ledger.

Eligible plans include a weekly usage allowance. Past that pool, work is billed on demand from model and token cost ([Grok Bot FAQ](https://docs.x.ai/grok-bot/faq)). No published page prints the allowance as dollars, credits, or runs. There is no Grok Bot-specific spend cap yet ([teams and enterprises](https://docs.x.ai/grok-bot/teams-and-enterprises)).
...
| Why did usage explode | [Grok Bot cost](/blog/grok-bot-cost) | It will not trace frequency, retries, or long documents |
| What do plans cost today | [What AI bots cost](/blog/what-ai-bots-cost) | It will not reprint the price grid |
| How do I prove what the bot proposed | [No audit view yet](/blog/grok-bot-no-audit-log-yet) | It will not stand in as an action log |
...
Score one job with one boundary. [Inbox Triage](/bots/inbox-triage) never sends. You already know how long a morning of drafts takes. A bot that "handles operations" has no clock. Split it until the hours have one.

All bots share one persistent cloud computer assigned to the user, not to a bot ([computer and apps](https://docs.x.ai/grok-bot/computer-and-apps)). Overflow follows that grain. Score one bot at a time. Averaging the roster hides the bot that burned the pool while another returned the hours.
...
[Mail Cleanup Assistant](/bots/mail-cleanup-assistant) misses cheaper: rejecting a label costs seconds, not a rewrite. Still flag a proposed permanent delete, even when the boundary blocked it. The proposal took review time.
...
There is no Grok Bot-specific spend cap. Scoring as if a ceiling will fire is scoring a product that does not exist. Zero overflow plus real hours avoided is a keep for this window, not a promise about next week.

If you cannot tell which bot wrote the overflow, log the account line and write allocation unknown. Do not invent "this inbox bot caused sixty percent." There is no audit view of Bot actions to support a split ([teams and enterprises](https://docs.x.ai/grok-bot/teams-and-enterprises)). A keep is weaker when allocation is unknown. A pause is easier to defend.
...
The charter matched [Inbox Triage](/bots/inbox-triage): classify, draft, never send. An approval controls the proposed action. It does not reverse work already completed ([approvals, security, and privacy](https://docs.x.ai/grok-bot/approvals-security-and-privacy)).
...
[Chief of Staff Briefing](/bots/chief-of-staff-briefing) needs a different column A: minutes you spent assembling a pack by hand. Time three packs before you score it. Inventing "this would have taken two hours" after you liked the output is how every briefing looks like payback.
...
Ask is the gate. Keep the draft rejectable. [Standup Scribe](/bots/standup-scribe) posts only to your own DM, never to a shared channel, so a bad note stays off the team while you still measure typing time.
...
If the job cannot sit on ask (a calendar change that already landed, a purchase already completed), this method is the wrong tool. Approvals do not undo completed work. Pause. Read [approval reversibility](/blog/grok-bot-approval-rules-reversibility). Come back only on jobs that still stop in front of you.
...
The strongest objection: you already pay Cursor Pro+, Ultra, Teams, SuperGrok Plus, or Heavy, so every bot is incremental-cost-zero, so this grok bot paid for itself the first morning it drafted anything.
...
It also treats the roster as one product. [Lead Scout](/bots/lead-scout) can empty the week in a long browser pass while the inbox drafter looks innocent. "The subscription paid for itself" scores the account, not this bot. Score the scout on a separate sheet.

If you are not on an eligible plan, the objection does not apply. Cursor Hobby, Cursor Pro at 20 USD, and SuperGrok at 30 USD do not include Grok Bot. The cheapest individual paid door is Pro+ at 60 USD, checked 25 August 2026 on [cursor.com/pricing](https://cursor.com/pricing). Upgrade math lives on [Is Grok Bot worth it](/blog/is-grok-bot-worth-it). This page starts after the door is open, or on a timed trial job.
...
Pause from the phone if needed. At a desk, read the last twenty records, coarsen or delete the clock, then rewrite the charter. [Operating without a spend cap](/blog/grok-bot-no-spend-cap-ops) is the Friday roster ritual. This pause is one bot, two failed payback windows.
...
Bill shape (frequency, retries, long documents) is [Grok Bot cost](/blog/grok-bot-cost). Published plan prices and unpublished figures are [What AI bots actually cost](/blog/what-ai-bots-cost). Proof of what the bot proposed is [Grok Bot has no audit view yet](/blog/grok-bot-no-audit-log-yet). Hours versus overflow will not tell a controller which vendor name was invented on 20 August.

[How to stop overspending](/blog/how-to-stop-grok-bot-overspending) is the hour you pause everything. [No spend cap ops](/blog/grok-bot-no-spend-cap-ops) is the Friday roster test. Use them when overflow is the emergency. Come back when the clocks are quiet and you want to know whether the surviving bot still returns hours.

SpaceX acquired xAI (announced 2 February 2026). SpaceX acquired Anysphere/Cursor (closed 14 August 2026). Neither added a Bot-specific spend cap, an allowance dollar figure, or an audit view.
...
Coming soon, not shipped: a team-level ceiling on local execution, and an admin Kill that deletes the VM while durable storage is kept. Kill is not a ledger. Do not log coming-soon rows.
