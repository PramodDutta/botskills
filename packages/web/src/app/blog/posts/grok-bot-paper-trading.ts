...
From the phone app (iPhone or Android) you can approve steps and pause or resume a routine, but not edit it. Editing and testing a routine still need the desktop app; the phone can now show run history and delete a routine. The desktop app runs on macOS, Windows and Linux; the phone app runs on iPhone, Android and, through the iOS app, iPad. The agent runs on a managed Linux VM as a non-root user, which is not a Linux desktop app. Eligibility widened on 21 August 2026. Cheapest paid path as of 23 September 2026: Cursor Pro at $20/mo. Cursor Hobby, the free plan, does not include it. Confirm prices on the vendor's current page. There is no Grok Bot-specific spend cap, but the account-level On-demand monthly limit applies. Weekly allowance then on-demand from model and token cost. Never invent a dollar figure for the allowance. See [Grok Bot cost](/blog/grok-bot-cost).
...
Do not complete broker 2FA on this computer, even to export. A broker session can Buy. Copy marks at your desk. Drop marks.txt in the folder. Do not send a one-time code in ordinary chat. The field-level procedure for a 2FA wall you did intend is in [Grok Bot hit a 2FA prompt](/blog/grok-bot-2fa-prompt). On this desk you do not intend it. [Mail Cleanup Assistant](/bots/mail-cleanup-assistant) still sends nothing. If it can read the mailbox that receives broker one-time codes, you have a live second factor and a funded session in the same cookie jar. Privacy Mode (Legacy) blocks Grok Bot entirely. Two admin controls have shipped since the August docs previewed them: a team-level ceiling on local execution for Teams and Enterprise admins, and Terminate, which lets Enterprise organization admins delete a member's computer while the durable disk is kept. Terminate brings synced logins back, so it will not erase a broker cookie.
