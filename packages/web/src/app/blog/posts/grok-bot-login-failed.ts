...
a SuperGrok nobody linked, Legacy Privacy Mode is on, or you completed a personal
...
and Intel), Windows (x64 and Arm64), Linux (x64 and Arm64), iPhone on iOS 18
or later, iPad on iPadOS 18 or later, and Android 9 or later
([FAQ](https://docs.x.ai/grok-bot/faq)). It does not prove access. An OS
below those versions is a platform miss, not grok
...
is whether the Cursor user who just signed in holds a paid Cursor plan (Pro,
Pro+, Ultra), a Cursor Teams seat, a linked individual SuperGrok, SuperGrok
Plus, SuperGrok Heavy, or X Premium+, or a one-time trial
([FAQ](https://docs.x.ai/grok-bot/faq),
[teams and enterprises](https://docs.x.ai/grok-bot/teams-and-enterprises)).
Hobby, SuperGrok Lite, and a SuperGrok nobody linked are not that list.
Confirm live SKUs on [Cursor pricing](https://cursor.com/pricing) and
[xAI pricing](https://x.ai/pricing) before you pay. Prices below were
rechecked on 23 September 2026.
...
| Invoice line (checked 23 Sep 2026) | On the Grok Bot FAQ list | What login will do |
|---|---|---|
| Cursor Hobby (free) | No | Sign-in can complete. Access will not. |
| Cursor Pro at $20 | Yes. Cheapest paid individual door | Succeeds if privacy and identity also match. |
| Cursor Pro+ at $60 | Yes, more weekly usage than Pro | Succeeds if privacy and identity also match. |
| Cursor Ultra at $200 | Yes | Succeeds. Ultra is not required for the bot. |
| Cursor Teams | Yes, every member | Succeeds only on the member identity that holds the seat. |
| SuperGrok (individual) | Yes, by linking | Link it from the Grok Bot plan screen; it grants usage and is not a Cursor plan |
| SuperGrok Plus or Heavy | Yes, by linking | Succeeds once linked to the Cursor user you sign in with. |
...
page. Do not invent one. On a self-serve Cursor Teams plan every member has
Grok Bot, with no Premium seat and no admin request. A Premium seat is not a
better bot.

Cursor Hobby is cause one. It is the free plan, and it does not include Grok
Bot. The editor can work, the installer can finish, Sign In with Cursor can
use this morning's Google account, and the product still refuses. Until
September, Cursor Pro at $20 failed the same way; every paid Cursor plan now
includes Grok Bot. The check is not "is this person a Cursor customer." It is
"is this person on the FAQ list."

Cursor Pro at $20 a month is the cheapest documented paid path for one person. The
$40 step from Pro to Pro+ buys more weekly usage, not access. A SuperGrok on
your xAI account does nothing for a Hobby login until you link it from the
Grok Bot plan screen, and a link is permanent. Do not buy a SuperGrok tier
solely to unblock a Cursor editor seat. If you are sampling,
...
only the account-level On-demand monthly limit, and no model picker
([cost](/blog/grok-bot-cost),
[spend cap](/blog/grok-bot-spend-cap-and-token-burn)). If the invoice already
says Pro, Pro+, Ultra, Teams, a linked SuperGrok, or trial, jump to privacy
and SSO.
...
[least privilege](/blog/least-privilege-bots). Individual accounts and self-serve Teams still have no audit view of Bot actions; Enterprise has audit logs and Action Recording. Do not wait for a log of the login failure inside Grok Bot.

## Finish the organization SSO path on the member account that holds the seat

Cause three is membership. Self-serve Cursor Teams seats
...
for Cursor Teams for twenty people. You can still grok bot login failed if
the app used the Gmail you created in 2014, the one that holds Cursor Hobby
for evening side projects.
...
still says Pro at $20. That was the right diagnosis on 27 August. Since
September every paid Cursor plan includes Grok Bot, so today the same
invoice would let her in, and a refusal would point at Hobby, privacy, or
the wrong user.
...
| 14:31 | Opened the Cursor invoice | Plan name is Pro, $20, not on the August FAQ list. |
...
|---|---|---|
| Sign-in returns you to an empty product, invoice says Hobby | Cause one: closed plan | Reinstall, architecture swap, a GitHub zip |
...
after a successful login. There is still no Grok Bot-specific spend cap, only
the account-level On-demand monthly limit.
...
| Which SKU do I buy once I accept I am on Hobby? | [Cheapest way in](/blog/cheapest-way-into-grok-bot) | Restate the full shopping order |
...
active. Hobby fails. Start the trial or move to Pro. Reinstalling is not
a check.
...
Store search for Grok Bot on iPhone, iOS 18 or later, or Google Play on
Android 9 or later. A second copy of that file will not change Hobby into
Pro. A new laptop works when the old machine ran an unsupported OS version.
That was no client, not login failed.
...
old plan was Hobby on the same user. Ultra is eligible. So is Pro at $20.
...
beyond "the app launched," SKU shopping beyond "Hobby is closed," or a
stuck checkout after you are in.

Enterprise access is rolling out. If org SSO succeeds, Legacy Privacy Mode
is off, the seat is Cursor Teams, and it still refuses, that is
...
can. The phone app can pause or resume a routine and approve steps, but not edit one. Missing edit controls after a successful phone
