...
This page is not the generic picker. [How to pick the first job](/blog/pick-the-first-grok-bot-job) scores any team's nominees on reversibility, evidence, blast radius, and who still owns the irreversible step. Stay there if you are a five-person product company arguing among inbox, outbound, and competitor watch. Stay here if you are a small nonprofit: board pack, thank-you drafts that never send, never donate-form automation, never donor CRM writes. Primer: [what a Grok Bot is](/blog/what-is-a-grok-bot). Disk: [the shared computer](/blog/grok-bot-shared-computer-security).
...
Write the week before you name a bot. If the first string you type is Development Bot, you already skipped the split. The named bot is a screen. Screens are not security boundaries. [Do not use separate bots as a security boundary](/blog/grok-bot-not-a-sandbox) is the engineer page. Renaming the helper does not keep it off the donate form. A grok bot for nonprofits is a role page: file-to-file drafts, human hours, and clicks that never move onto the Agent Computer.
...
| Labels on a dedicated alias with no history | You | Not week one | Connect is a cookie. Fill [the pre-flight sheet](/blog/grok-bot-preflight-checklist) first |
...
Thank-you copy is the first reversible job: you already write it, you can grade it in twenty minutes, and a wrong paragraph is a file you can delete. A board briefing is job two, the same shape as [Chief of Staff Briefing](/bots/chief-of-staff-briefing): one dated document, sources on disk, no send.
...
[Inbox Triage](/bots/inbox-triage) is the mail shape, not the mailbox. Steal the stop: drafts wait. Do not steal the connector. hello@ is the public address on the website, not a development inbox.
...
hello@ looks like the thank-you job. Connecting Gmail plants a session on one persistent cloud computer assigned to the user, not to a bot. Every other bot can open it. Deleting the development bot does not remove it. [Mail Cleanup Assistant](/bots/mail-cleanup-assistant) needs the cookie. [Lead Scout](/bots/lead-scout) and [Churn Watch](/bots/churn-watch) inherit the same jar. [Grok Bot and Gmail](/blog/grok-bot-gmail) is the catalogue after you choose to connect. Day one is files.
...
The [safety checklist](/blog/grok-bot-safety-checklist) should fail this role the moment a send, pay, or CRM plugin is connected. [Least privilege](/blog/least-privilege-bots): this week, connect nothing that talks to a donor.
...
Copy the board agenda and last minutes as PDFs. Do not paste a Drive link. Do not leave Drive signed in. [Grok Bot and Google Drive](/blog/grok-bot-google-drive) is a later conversation. If a row will not parse, band it UNOPENED. Fluency that invents a fund name is a failed run. Hosted MCP sign-in tokens stay with Cursor's backend, not on the computer. That is not a reason to connect the CRM this week.
...
The freeze on verbs is load bearing. "I thanked them" is a send. "I stamped Thanked" is a CRM write. "I tested the button" is a payment. An approval is a gate in front of the next click. It does not reverse a gift that already processed. [Approvals, rules, and reversibility](/blog/grok-bot-approval-rules-reversibility) is the general form.
...
If the chair wants conversion, hire a human for the form or use the form vendor's own tools, confirmed on that vendor's current page. If the chair wants CRM writes, that is a different product. If the chair wants hello@ empty, [Inbox Triage](/bots/inbox-triage) is still draft-only, and [the pre-flight checklist](/blog/grok-bot-preflight-checklist) still has to pass. The generic version of this objection lives on [pick the first job](/blog/pick-the-first-grok-bot-job): glamorous irreversible work wins the meeting. Here the stranger is a donor. Confirm your privacy policy before any CSV lands on a cloud disk.
