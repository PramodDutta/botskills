...
Scheduled work needs a pause path before an emergency. Dev writes who can pause the routine, what happens to an in-progress run, how the next window is chosen after resume, and when accumulated drafts expire. On the phone app, verified product behavior permits pausing, resuming and approving; Editing and testing a routine still need the desktop app. That platform detail affects incident procedure, so the owner knows what can and cannot be changed away from a desktop.
