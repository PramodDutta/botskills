...
export const post: BlogPost = {
  title: 'Replace a VA Browser Shift with a Grok Bot',
  description:
    'Replace a VA browser shift with a grok bot only for reversible research and drafts. Teach-by-demo is 10 minutes, browser only, no microphone, not on iPhone. Never send.',
  date: '2026-08-28',
  category: 'Guide',
  content: `
...
This page is the replacement decision. It is not [Grok Bot vs Zapier](/blog/grok-bot-vs-zapier), which asks whether the job is a graph. It is not [teach Grok Bot by demonstration](/blog/teach-grok-bot-by-demonstration), which walks the recorder. The question here is when you can replace a VA shift with a grok bot, and what must stay with a named human. The winner is a dated research pack on disk. A person still publishes. Never send.
...
| Deterministic export | The same CSV from the same URL every night | Usually a Zap, not a bot | Confirm on the vendor page, then see [Grok Bot vs Zapier](/blog/grok-bot-vs-zapier) |
...
The replacement you ship is smaller than the shift you bought. The bot loads public pages, extracts named regions, writes a pack, and stops. [Competitor Website Watch](/bots/competitor-website-watch) never signs up, logs in, or submits a form. [Competitor Pricing Watch](/bots/competitor-pricing-watch) never starts a trial to reveal a price. Steal the boundary. Do not steal a fantasy that the bot is now the VA.
...
Name the human who publishes. Write the name in the charter. "The team" is how a pack sits unread until Friday. Helene, in the worked example below, still posts at 07:30. The bot never opens Slack as a poster. [Chief of Staff Briefing](/bots/chief-of-staff-briefing) is the same shape on a different input: draft on disk, human on the wire.
...
Helene's 07:40 deny is the demonstration. The Slack post waited. The form did not. Park send, pay, delete, publish, form submit, and trial start so those clicks are never proposed. Let the pack finish. [Draw the approval line on reversibility](/blog/grok-bot-approval-rules-reversibility) is the principle. [How to set Grok Bot approvals](/blog/how-to-set-grok-bot-approvals) is the walkthrough.
...
Twenty run records is not an audit log. If you need thirty nights of evidence, export the pack files to a folder you own. Schedule one harvest, one bot, one clock. 02:00 is an arbitrary example. Do not stack a Slack poster on the same bot "because it is the VA bot." [How to schedule a Grok Bot routine](/blog/how-to-schedule-a-grok-bot-routine) is the clock. Eligibility widened on 21 August 2026, so older "rare seat" posts are stale. Confirm on [docs.x.ai](https://docs.x.ai/grok-bot/faq) and current pricing pages before you buy.
...
If the overnight work is a deterministic graph (same CSV, same URL, same row shape), the ops lead is on the wrong page. Use [Grok Bot vs Zapier](/blog/grok-bot-vs-zapier) or [Grok Bot vs n8n](/blog/grok-bot-vs-n8n). Variance is why a harvest bot can survive a renamed button. Variance is also how it discovers a form.
...
If the job is "when this file appears, copy these columns," you are on the graph side. Go to [Grok Bot vs Zapier](/blog/grok-bot-vs-zapier).

If the job is inbox labeling or anything that can reach Send, you are on mail. [How to set Grok Bot approvals](/blog/how-to-set-grok-bot-approvals) and [Inbox Triage](/bots/inbox-triage) are the never-send pattern. Do not teach mail by demonstrating the VA's full "handle the inbox" ritual. That ritual includes reply.
...
If you already decided the harvest is in scope and you now need the camera, leave for the twin: [teach Grok Bot by demonstration](/blog/teach-grok-bot-by-demonstration). Ten minutes, browser only, no microphone, not on iPhone, draft skill. Come back if the recording starts to look like the whole shift.

If you need the filter for competitor pages rather than the VA decision, [How to build a Grok Bot that can monitor competitors](/blog/grok-bot-to-competitor-monitoring) is that filter. SpaceX acquired xAI (announced 2 February 2026) and closed the Anysphere (Cursor) acquisition on 14 August 2026. Do not tell finance that xAI acquired Cursor.
...
**Keep reading:** [Teach Grok Bot by Demonstration: Ten Minutes, Browser Only, Draft Skill](/blog/teach-grok-bot-by-demonstration), [How to Build a Grok Bot That Can Monitor Competitors](/blog/grok-bot-to-competitor-monitoring), [Draw the Approval Line on Reversibility, Not Task Size](/blog/grok-bot-approval-rules-reversibility).
