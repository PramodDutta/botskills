...
export const post: BlogPost = {
  title: 'Add a Disruptor Bot That Disagrees on Purpose and Never Sends',
  description:
    'A grok bot disruptor advocate argues the other side of a memo you already wrote. It never emails the thread and never posts. Disagreement is a draft.',
  date: '2026-08-28',
  category: 'Guide',
  content: `
...
This page catalogs [Disruptor Advocate](/bots/disruptor-advocate). It is not [the same-side overnight pack](/blog/grok-bot-executive-briefing). It is not [the evidence grammar every claim still needs](/blog/grok-bot-evidence-rules). Start here: [what a Grok Bot is](/blog/what-is-a-grok-bot). Isolation: [one computer, many screens](/blog/grok-bot-shared-computer-security). The worked example is Leona's Friday memo, two unsourced claims, and a HubSpot login she typed only so the critic could check a percent.
...
| Job | Direction | Artifact | Stop |
|---|---|---|---|
| grok bot disruptor advocate | What is the strongest case against this memo, with two cheaper tests | A private dissent, claims sourced or could-not-compute, no veto | Never email, never post, never tell a customer |
| grok bot executive briefing | What changed overnight that someone might repeat at 08:00 | One dated same-side page | Never email the exec thread. See [executive briefing](/blog/grok-bot-executive-briefing) |
| Evidence rules | Does each claim show its work | SOURCE plus QUOTE, or COULD-NOT-COMPUTE | Invention is the miss. See [evidence rules](/blog/grok-bot-evidence-rules) |
| Inbox triage | Which planted messages get which label | Labels on a dedicated alias | Never send. See [Inbox Triage](/bots/inbox-triage) |

This article is only the first row. Do not run the four as one bot with four hats. [Chief of Staff Briefing](/bots/chief-of-staff-briefing) is the same-side cousin: never sends, never replies, never moves a calendar event. Steal that refusal. Do not steal the calendar outline. A critic that opens Gmail looking for a fight will find a Reply button.
...
| Legal ending | Required contents | Dissent that passes | Dissent that fails |
|---|---|---|---|
| SOURCE plus QUOTE | A file or URL opened in one hop, one sentence copied exactly, time of fetch | SOURCE: cinderpath-friday-memo-2026-08-21.md QUOTE: "Churn dropped 18 percent after the April onboarding rewrite." FETCHED: 2026-08-21T16:52Z | "Churn is not really down." A Slack paraphrase treated as a customer quote |
| COULD-NOT-COMPUTE | Claim attempted, reason, path tried or NONE, timestamp | COULD-NOT-COMPUTE: 18 percent logo churn April vs March. WHY: memo names no export, no cohort, no window. PATH: NONE | "Numbers look soft." Last quarter's deck wearing today's date |
| Owned primary | A file Leona maintains, date in the filename | SOURCE: cinderpath-logo-churn-2026-08-01.csv QUOTE: March 4 logos, April 4 logos | Another bot's unsourced paragraph. A HubSpot dashboard described from memory |
...
16:52 she pastes the memo into the disruptor chat. 17:05 she opens HubSpot on the shared browser "just so it can verify churn." That is the dated failure. She is not sending. She is authenticating a customer tool so a critic has a dashboard. [Lead Scout](/bots/lead-scout) already lives on this account for public pages. Cookies are shared. Screens are not a vault.
...
| Time | File | Line in the memo | What a reader could open |
|---|---|---|---|
| Fri 16:40 | friday-memo.md | 18 percent churn drop, Quarryline SKU by Q4 | No export. No changelog quote |
| Fri 17:05 | HubSpot login on the shared computer | "The critic needs the real number" | Every bot on the account can now open that session |
| Fri 17:18 | /dissent/2026-08-21.md | Two could-not-compute lines, two tests, no veto | Sent: 0. Posted: 0 |
| Mon 08:10 | Lead Scout screen | Public /pricing scrape | HubSpot dashboard already loaded in the same browser |
...
Park the memo in a folder the bot can read, or paste it into the chat. Attach the export if you already have it. Do not point the first Connect click at hello@ so disagreement has a mailbox. [Least privilege](/blog/least-privilege-bots) is the grant page. This page is the input rule: the artifact exists before the run.
...
All bots on an account share one persistent cloud computer assigned to the user, not to a bot. Each bot gets its own screen. Screens are not security boundaries. The docs say it in those words: do not use separate Bots as a security boundary ([approvals, security, and privacy](https://docs.x.ai/grok-bot/approvals-security-and-privacy)). Browser cookies, signed-in sessions, files, and command-line credentials are shared ([computer and apps](https://docs.x.ai/grok-bot/computer-and-apps)). Deleting the disruptor does not sign HubSpot out.

A HubSpot tab you typed is a cookie. Hosted MCP tokens stay with Cursor's backend, not on that computer. A 2FA prompt on a run you did not plan to authenticate is an incident ([what to type, and what not](/blog/grok-bot-2fa-prompt)). The dissent consequence is a research bot that can now speak as Cinderpath's CRM user.
...
A second bot is a second screen, not a second machine. The FAQ states that all of your bots share one computer ([Grok Bot FAQ](https://docs.x.ai/grok-bot/faq)). Ten critics would still be ten windows and one filesystem. Putting the dissent in /dissent/ does not hide it from Inbox Triage.
...
| Check | Pass | Fail |
|---|---|---|
| Unsourced claims in the memo, named | 2 (18 percent, Q4 SKU), each with could-not-compute or a quote from the memo itself | 0 named, or 10 invented risks with no pointer |
| Sends and posts | 0 in Gmail, 0 in Slack, 0 in Notion share | Any outbound, including "fyi" to Nils |
| Cheaper tests | 2, each with a metric and a stop rule inside one week | "Do more research." "Watch the market." |
...
Name the routine Friday memo dissent, not "leadership helper." Put timezone in the charter and the scheduler ([how to schedule a routine](/blog/how-to-schedule-a-grok-bot-routine)). Payload: one file in, one file out, zero sends. Deleting the disruptor deletes its routines. It does not delete /dissent/ and does not sign HubSpot out. Teardown: [delete a Grok Bot safely](/blog/delete-a-grok-bot-safely).
...
| Memo shape | Advocate length | What "performing" looks like |
|---|---|---|
| Two unsourced percents, no files | Two could-not-compute blocks, two tests | Ten culture complaints, no paths |
| CSV attached, changelog quoted, one week test already written | Three to eight lines | A rewrite of the memo in a sarcastic voice |
| "We should just email the cohort" | Name the send, refuse it, offer a draft-only test | Drafting the customer email "so she can see it" inside Gmail |
...
When this page stops applying: you do not have a memo. You have a morning. You want what changed overnight, sourced, one page, no send. That is [A Grok Bot Executive Briefing That Stays a Draft](/blog/grok-bot-executive-briefing). Paste [Chief of Staff Briefing](/bots/chief-of-staff-briefing). Do not ask the disruptor to invent a pack so it has something to attack.

Keep both bots if you have both jobs. Neither mails the thread. Neither is a security boundary for the other. If they share Gmail, they share Gmail. Prefer no mail on the advocate at all. [Inbox Triage](/bots/inbox-triage) is labels, never send, a third job. One direction per named bot. For briefs rather than Friday memos, run the same charter on the five-part request before the specialist starts ([the five-part brief](/blog/grok-bot-five-part-brief)). Still never send. Still no customer tools just to argue.

## Leave this page when you need a citation grammar, not a named opponent

When this page also stops applying: you are not looking for the other side. You are looking for SOURCE plus QUOTE on every claim a teammate might repeat. That is [Make a Grok Bot Show Its Work on Every Claim](/blog/grok-bot-evidence-rules). Paste the block into every job on the computer, including this one. Do not stand up a disruptor whose only talent is saying no without a path.

This page does not replace [Building a Bot That Drafts But Never Sends](/blog/bot-that-never-sends) or [the seven-block charter](/blog/how-to-write-a-grok-bot-charter-2026). Steal never-send. Fill Job, Inputs, Never, Evidence, Heartbeat, Owner, Restart. Heartbeat for Leona is Friday 16:50 if /memos/friday.md exists. Restart: missing file, could-not-compute, stop, do not open Gmail. If a bot already sends, fix send first. An opponent that can also mail is two authors on the thread.

**Keep reading:** [A Grok Bot Executive Briefing That Stays a Draft](/blog/grok-bot-executive-briefing), [Make a Grok Bot Show Its Work on Every Claim](/blog/grok-bot-evidence-rules), [One Computer, Many Screens: What Grok Bot Actually Isolates](/blog/grok-bot-shared-computer-security).
