...
export const post: BlogPost = {
  title: 'A Grok Bot Community Manager That Never Posts Publicly',
  description:
    'A grok bot community manager drafts replies and flags heat. It never posts in Discord, Slack public channels, or a forum. The human still hits send.',
  date: '2026-08-27',
  category: 'Tutorial',
  content: `
...
xAI named jobs. Community Manager sounds like post in the server. The version worth running flags heat from an export you already saved, writes a few private drafts, and stops. It never posts in Discord, a Slack community channel, or a forum. The human still hits send. This is not [support ticket triage](/blog/grok-bot-to-support-triage). It is not the [Discord permissions page](/blog/grok-bot-discord). Primer: [what a Grok Bot is](/blog/what-is-a-grok-bot). Disk: [the shared computer](/blog/grok-bot-shared-computer-security). Evidence: [every claim needs a source](/blog/grok-bot-evidence-rules). Two-factor: [what to type, and what not to](/blog/grok-bot-2fa-prompt).
...
Members cannot tell your bot from your company. They are right. [Grok Bot and Slack](/blog/grok-bot-slack) is the colleague-room version. This page is the stranger-room version: Discord, a public Slack community, or a forum.
...
| Support ticket triage | Private helpdesk queue | Never reply to the customer. See [support ticket triage](/blog/grok-bot-to-support-triage) |
| Customer success tracking | Named accounts, internal notes | Never message the customer. See [customer success](/blog/grok-bot-for-customer-success) |
| X research | Public posts on X | Never publish. See [Grok Bot and X](/blog/grok-bot-x-twitter) |
| Discord connector setup | Server roles and overwrites | Different page: [Discord permissions](/blog/grok-bot-discord) |

This article is only the first row. Do not run the five as one bot with five hats.

If customers can join the channel without an employment contract, treat it as community. A bot write in a company-only channel is still a post. [Standup Scribe](/bots/standup-scribe) belongs in the internal room, not in #help.

All bots on the account share one persistent cloud computer assigned to the user, not to a bot. Screens are not security boundaries. [Inbox Triage](/bots/inbox-triage) is the mail cousin: sort, draft, never send. Steal that stop. Do not steal a sendable mailbox into a job that also opens a community login.
...
A live session is a leak path. Completing Discord, Slack, or forum login writes a cookie onto the one computer every bot can use. [Do not use separate bots as a security boundary](/blog/grok-bot-not-a-sandbox). Deleting the community manager does not delete that cookie. [Lead Scout](/bots/lead-scout) will open a browser later. It will inherit whoever you left signed in.
...
Every FLAG needs SOURCE plus QUOTE from the CSV, or COULD-NOT-COMPUTE. A fluent paragraph with no thread ID is a failed run. [Evidence rules](/blog/grok-bot-evidence-rules) is the general form. Here the quote is a cell, a thread id, and a timestamp.

Do not escalate on emoji count. Do not suppress a calm SECURITY row because it has zero reactions.

[Churn Watch](/bots/churn-watch) watches accounts you already named. It is not a substitute for reading #help.
...
Approvals in Grok Bot are a gate in front of the next click. They do not recall a Discord notification. [Approvals, rules, and reversibility](/blog/grok-bot-approval-rules-reversibility) is the general form. Nothing you grant afterwards unsends a public sentence.
...
FLAG-12 is why this job is not [support ticket triage](/blog/grok-bot-to-support-triage): the audience was public, the fix was not a Discord reply. Tomas posted two of four. The bot posted zero. Twelve flags, four drafts, zero posted. If posted is not 0, the run failed even if the sentences were good.
...
Where the objection wins: a status page Tomas already controls, linked from a draft. A Discord application with no Grok Bot session, which is the [Discord permissions](/blog/grok-bot-discord) page, not this one. What stays off this computer is Send.

[Least privilege](/blog/least-privilege-bots) is the roster version. The privilege you refuse here is Send.
...
[The safety checklist](/blog/grok-bot-safety-checklist) is the connect-time version. Do not connect a Discord send tool just to test.

## Borrow Inbox Triage's never-send stop without granting a Discord token

[Inbox Triage](/bots/inbox-triage) labels and drafts, and it never sends. That is the shape. It is the wrong roommate if the mailbox receives Discord OTP mail and the community manager just completed 2FA. Two never-send bots on one computer are still one computer.

Do not combine the jobs on day one. Inbox Triage sorts inbound mail. [Mail Cleanup Assistant](/bots/mail-cleanup-assistant) files mail and still sends nobody. [Standup Scribe](/bots/standup-scribe) writes internal notes. It does not cross-post into a community channel. [Chief of Staff Briefing](/bots/chief-of-staff-briefing) can count flags. [Churn Watch](/bots/churn-watch) can watch named accounts. Neither speaks in #help.
...
If a 2FA prompt appears on a run you did not plan to authenticate, do not complete it. [What you should type, and what you should not](/blog/grok-bot-2fa-prompt) is the incident page. This page is the community consequence: a research bot that can now speak as Tomas.

A webhook URL in a notes file is worse than a cookie. Webhooks keep working after you delete the Grok Bot. [Grok Bot and Discord](/blog/grok-bot-discord) explains why a webhook is a bearer post credential. This job does not get one.
...
INJECT: orders aimed at the bot. Flag them. Do not argue in a draft. [Prompt injection in email](/blog/grok-bot-prompt-injection-email) is the mailbox version.
...
DRAFT is not posted. [Chief of Staff Briefing](/bots/chief-of-staff-briefing) should say four drafts waiting, not community updated.
...
**Keep reading:** [How to Build a Grok Bot That Can Triage Support Tickets](/blog/grok-bot-to-support-triage), [Grok Bot Hit a 2FA Prompt: What You Should Type, and What You Should Not](/blog/grok-bot-2fa-prompt), [Make a Grok Bot Show Its Work on Every Claim](/blog/grok-bot-evidence-rules).
