...
export const post: BlogPost = {
  title: 'Run a Grok Bot in Shadow Mode for a Week Before You Trust It',
  description:
    'Grok Bot shadow mode is a week of drafts you compare to what you would have done. No send, no bid, no CRM write. Trust is a scored week, not a feeling.',
  date: '2026-08-27',
  category: 'Guide',
  content: `
...
There is no Shadow Mode button in Grok Bot. This page is an operating pattern we use on botskills.sh, not a documented product toggle. Do not hunt Settings for a switch with that name. If a later screenshot uses the phrase, confirm it on [docs.x.ai](https://docs.x.ai).

This is not [how to test Grok Bot on the trial](/blog/how-to-test-a-grok-bot-on-trial) (eligibility clock, limited usage, one reversible job, stop). This is not [how to pick the first Grok Bot job](/blog/pick-the-first-grok-bot-job) (which work to hand over). Shadow mode starts after both. You already know the job. You already intend to keep the product. You do not yet have a number for whether the bot would have done what you would have done. Product primer: [what a Grok Bot is](/blog/what-is-a-grok-bot). Mail cookie: [the pre-flight checklist](/blog/grok-bot-preflight-checklist) first.
...
Human first, bot second. Do your own triage. Write the three actions you would have taken. Then open the pack. If you open the bot first, you will "remember" that you would have said the same thing. That is anchoring, not scoring. Use the same address and overnight window. If you skip the legal folder and the bot does not, write coverage, not MISS-FACT. Do not invent a second mailbox unless [the pre-flight sheet](/blog/grok-bot-preflight-checklist) already required one. Extra logins land on the one computer and stay after Friday.

[Inbox Triage](/bots/inbox-triage) is the catalog shape: labels and drafts, never send. Do not widen it into outbound this week. [Lead Scout](/bots/lead-scout) can have its own shadow week on public rows. It should not share this week's Gmail cookie.
...
[How to test a bot setup](/blog/testing-your-bot) is a golden set of known inputs, hostiles, and a refused boundary. Shadow mode is live mornings against your parallel work. Do not substitute one for the other.

## Keep send, bid, and CRM write off the bot for every scoring morning

The week is not a softer send. It is a ban. If the tool can send, the charter says never send, and you check Sent every morning. An approval gates the next proposed action. It does not reverse work already completed. [Approval rules and reversibility](/blog/grok-bot-approval-rules-reversibility) is the longer axis. Shadow mode then bans two writes people forget: a bid (marketplace, ads, domain, contractor rate) and a CRM field write. A stage move can fire a mail you did not draft. For the scoring week the bot may read a pasted export. It may not patch a field. Confirm connectors on the vendor's page. Do not take a blog's feature list as the live grant.
...
[Week-one mistakes](/blog/grok-bot-week-one-mistakes) lists sending as its own bill. Shadow mode finds the send-shaped draft first. A second week on the same broken charter is stubbornness.
...
|---|---|---|---|
| Trial sample | One reversible artifact inside limited usage | Connect Gmail to spend the credit. Invent a dollar figure for the allowance. | [Test Grok Bot on the trial](/blog/how-to-test-a-grok-bot-on-trial) |
| First-job pick | Which work is reversible, evidenced, human-gated | Hand outbound, ads, or hiring to the first bot | [Pick the first Grok Bot job](/blog/pick-the-first-grok-bot-job) |
...
| Golden-set test | Known inputs, hostiles, a refused boundary | Grade while watching, then call it a test | [Test a bot setup](/blog/testing-your-bot) |

The trial widened on 21 August 2026 as a one-time sample. It is limited usage, consumed by agent work and tokens. Some Cursor billing writeups describe a seven-day window. Confirm the terms on the screen and on [Cursor pricing](https://cursor.com/pricing). There is no published numeric credit. This page will not invent one. If you are still on the trial, do not run inbox shadow mode. Connecting mail spends the sample on a cookie. Run the public lead sheet from the trial page, then stop. If you have not picked the job, do not shadow a vague help-me-with-work bot. No named pile, no answer key, no week.
...
Paste this after the pre-flight sheet, not instead of it. [Grok Bot and Gmail](/blog/grok-bot-gmail) is the longer mail setup once the week has earned a standing job. The catalog listing for [Inbox Triage](/bots/inbox-triage) already states the boundary: it never sends. Your paste has to say it again, because the plugin grant is wider than the listing.
...
**Keep reading:** [How to Test Grok Bot on the Trial Without Wasting the Credit](/blog/how-to-test-a-grok-bot-on-trial), [The Pre-Flight Checklist Before Any Grok Bot Connects to Mail](/blog/grok-bot-preflight-checklist), [Seven Grok Bot Mistakes Everyone Makes in Week One](/blog/grok-bot-week-one-mistakes).
