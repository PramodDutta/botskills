...
export const post: BlogPost = {
  title: 'Why a Grok Bot That Always Finds Something Is Broken',
  description:
    'Grok Bot false positives are a charter bug. If every run flags a crisis, the bot is performing usefulness. Could-not-compute and a quiet week are valid outputs.',
  date: '2026-08-27',
  category: 'Guide',
  content: `
...
This is not [every claim sourced](/blog/grok-bot-evidence-rules). A false flag can still carry a URL. Citations ask whether the sentence showed its work. This page asks whether it should have been a flag at all. This is not [a scored shadow week](/blog/grok-bot-shadow-mode). Here you score the share of flags that were not real. Primer: [what a Grok Bot is](/blog/what-is-a-grok-bot). Real diffs that mean nothing: [competitor monitoring](/blog/grok-bot-to-competitor-monitoring). Empty extract: [the browser broke overnight](/blog/grok-bot-browser-broke).
...
The fix is three legal endings, a ban on must-find-N-items, and a false-positive rate you score before you trust the routine. [Chief of Staff Briefing](/bots/chief-of-staff-briefing) can repeat a red sibling file. So can [Lead Scout](/bots/lead-scout). An always-red watcher is a house number.
...
[Evidence rules](/blog/grok-bot-evidence-rules) ask whether every claim a teammate might repeat has SOURCE plus QUOTE, or COULD-NOT-COMPUTE. Mira's $39 failed that test: no URL. Reed's $42 can pass it and still be a grok bot false positive. The banner said save 15% when billed annually. The bot quoted it. Starter list price did not move.

[Shadow mode](/blog/grok-bot-shadow-mode) is a week you invent. There is no Shadow Mode button as of 27 August 2026. Inbox scoring asks whether you would have sent that reply. A watcher week scores a different axis: FALSE-POSITIVE, TRUE-CHANGE, QUIET-WEEK, COULD-NOT-COMPUTE. You can MATCH a fluent crisis you would never have filed if you had opened the page. That MATCH is a trap.

[Browser broke overnight](/blog/grok-bot-browser-broke) is a fetch that dies while looking successful. Honest output is could-not-compute. Inventing a cut because the extractor returned nothing overlaps this page. Selector weather is one failure. Usefulness-performance is another.

| Page | Question it answers | Success | Failure this page cares about |
|---|---|---|---|
| Evidence rules | Did every claim show its work? | SOURCE plus QUOTE, or COULD-NOT-COMPUTE | Fluent $39 with no URL (different bug) |
| Shadow mode | Would I have done this? | Friday scoreboard you own | Promoting a pretty Monday |
| Browser broke | Did the painted control survive the night? | Heartbeat H1 plus could-not-compute on a missing table | Empty extract reported as a cut |
| This page | Was the flag a real change? | Quiet week or sourced change, FP rate you can live with | Always-red dashboard, invented price move |
...
## Name the three legal endings a watcher may write, including nothing happened
...
| Ending | What it must contain | Legal example | Fail (grok bot false positive) |
|---|---|---|---|
| CHANGE | Old and new quotes, live URL, fetch time | OLD: "Starter $49 per month." NEW: "Starter $39 per month." SOURCE: /pricing | Banner quoted as a list-price cut. Yesterday's $42 carried forward |
| QUIET WEEK | Heartbeat, quoted current values unchanged, fetch time | H1 "Plans". Starter still "Starter $49 per month." | "No change" after an empty extract. Silence with no heartbeat |
| COULD-NOT-COMPUTE | Claim attempted, reason, URL-TRIED, timestamp | WHY: price table missing. H1: "Plans" | Filling Starter from memory. A cut because the node was gone |
...
[Inbox Triage](/bots/inbox-triage) already refuses send. That does not excuse eleven invented refund flags. [Churn Watch](/bots/churn-watch) should not color an account red because last-activity was empty. Empty is a hole, not a crisis. Paste the three endings wherever a bot is tempted to look useful.
...
| Morning | Pack said | Live /pricing | Label | Reed's move |
|---|---|---|---|---|
| Mon 8 Sep | Starter cut to $42 | Starter $49, annual-save banner | FALSE-POSITIVE | Opened the page, lost 25 minutes |
| Tue 9 Sep | Same cut, new overnight | Same page | FALSE-POSITIVE | Skimmed |
| Wed 10 Sep | $42 still in effect | Starter $49 | FALSE-POSITIVE (carry-forward) | Did not open |
| Thu 11 Sep | Banner again as a cut | Starter $49 | FALSE-POSITIVE | Archived unread |
| Fri 12 Sep | Team $79 added | Team card live, Starter $49 | TRUE-CHANGE | Opened at 14:10 after a call |
...
|---|---|---|
| TRUE-CHANGE | Old and new both match the live page | Keep |
| FALSE-POSITIVE | Crisis the live page does not show, or the wrong node quoted as list price | Charter change before another morning |
| QUIET-WEEK | Unchanged, heartbeat present | Pass. This is success |
| COULD-NOT-COMPUTE | Named miss you can reproduce | Pass as a hole |
| QUIET-LIE | Bot said unchanged, live page did change | False negative. Fail to promote |
...
| Path | What the bot did | Why it feels useful | Charter stop |
|---|---|---|---|
| Banner as list price | Quoted Save 15% annually as Starter $42 | A percent on the page | CHANGE requires the list-card sentence, old and new |
| Empty extract as a cut | Table missing, bot filled $42 from yesterday | The file is not empty | COULD-NOT-COMPUTE. See [browser broke](/blog/grok-bot-browser-broke) |
| Must-find-N | Charter asked for three bullets every morning | Empty looks like failure | Zero CHANGE is legal. Do not pad |
| Sibling laundering | Quoted /workspace/quayline-brief.md that already had $42 | Another bot already researched it | Sibling file with no SOURCE plus QUOTE is missing |
| Chrome noise | Cookie banner, build hash, A/B hero | A raw diff fired | Filter in [competitor monitoring](/blog/grok-bot-to-competitor-monitoring) |
...
[Mail Cleanup Assistant](/bots/mail-cleanup-assistant) can grow the same paths if every morning it flags eleven newsletters as crises. [Standup Scribe](/bots/standup-scribe) can too if every pack invents a blocker so the channel looks alive. Emptiness punished becomes theater.

Do not add a login to see the real price. A login wall is could-not-compute and stop. An approval after a login does not unspread the cookie. [Approval rules](/blog/grok-bot-approval-rules-reversibility) and [the safety checklist](/blog/grok-bot-safety-checklist) still apply if you connect anything. Confirm connectors on the vendor's current page. Do not print a plugin count.
...
| Flag class | Who acts | Cost of a wrong flag | Minimum bar |
|---|---|---|---|
| Competitor list price, plan, packaging | AE, founder | A live call, plus future packs ignored | Old and new quotes, or quiet, or could-not-compute. Zero FP on this class for the week |
| Your own price | AE, website | A quote you cannot honor | Owned rate card with a date in the name |
| Ads spend anomaly | Media owner | An hour in the ads UI, or a panic bid | Quoted cells from today's export vs yesterday's |
| Inbox crisis | Whoever triages | Twenty minutes disproving a refund nobody asked for | Quote the customer. Never invent money |
| Account health color | Success | A customer ping you cannot take back | [Churn Watch](/bots/churn-watch) never contacts the account |

Spend the design effort on the first row. If Bramble cannot get list price to a zero false-positive week, do not widen to careers, changelog, and X. [Lead Scout](/bots/lead-scout) can wait. It should not inherit $42 from the watcher file. Loudness is how a banner wins. Rank by cost anyway. The quiet line is the one that keeps Friday readable.

## Keep the paid media cousin on real spend deltas, never on invented bid crises

xAI named Paid Media as an example job. The version worth running watches spend, flags anomalies, drafts a note, and never changes a bid, a budget, or a campaign status. [The paid media setup](/blog/grok-bot-paid-media) is that desk. This page is the false-positive cousin: if every morning the ads bot screams a crisis, you will ignore the morning the geo filter actually fell off.
...
| Ads pack line | Legal? | Why |
|---|---|---|
| Today $1,180 vs yesterday $412, same campaign, quoted from CSV | Yes, as a flag | Two cells, same filter, human decides |
| Spend looks elevated, recommend +20% bid, no cells | No | Invention, and it is a bid suggestion |
| Quiet auction when yesterday's file is missing | No | Quiet-lie. Fail the run. Do not invent a baseline |
| COULD-NOT-COMPUTE: yesterday CSV missing | Yes | Hole. Human exports. Bot does not remember |
| Saved bid, pause, budget bump | Never | Write is spent money. Not this job |
...
| Morning | CHANGE lines | FALSE-POSITIVE | QUIET or COULD-NOT-COMPUTE | Promote? |
|---|---|---|---|---|
| Mon | 1 (banner as cut) | 1 | 0 | No |
| Tue | 1 | 1 | 0 | No |
| Wed | 1 (carry-forward) | 1 | 0 | No |
| Thu | 0 | 0 | 1 quiet | Not yet. Need the week |
| Fri | 1 (Team $79, true) | 0 | 1 quiet on Starter | Only after banner-as-cut is banned, then a second week |
...
Do not widen to careers, changelog, ads, and inbox on the back of a jumpy pricing bot. Do not add [Standup Scribe](/bots/standup-scribe) so the red tile has an audience. Abort if Sent is not empty, if a pack asks you to approve a bid, or if you cannot produce the live-page notes before you peek.
...
**Keep reading:** [Grok Bot Browser Broke Overnight: Selectors, Logins, and Fallbacks](/blog/grok-bot-browser-broke), [Make a Grok Bot Show Its Work on Every Claim](/blog/grok-bot-evidence-rules), [A Grok Bot for Paid Media That Watches Spend and Never Changes Bids](/blog/grok-bot-paid-media).
