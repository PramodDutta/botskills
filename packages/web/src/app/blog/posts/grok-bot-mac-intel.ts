...
Intel is a row on the supported list, not a footnote under Apple silicon. The desktop client on that Mac can create bots, edit routines, test, read history, and delete. Editing and testing need a desktop app on macOS, Windows or Linux. They do not need an M1, M2, M3, or M4 in the lid.
...
| A dual-boot Ubuntu partition next to macOS | Still an Intel or silicon Mac when you are in macOS | The macOS client in macOS, or the Linux desktop app (x64) in Ubuntu | Either side is a Grok Bot desk since the Linux app shipped in September 2026 |
...
The industry moved. The FAQ did not add a silent Intel sunset. If a blog post shows only M-series laptops, that is the author's camera, not a platform cut. Documented clients are macOS (Apple silicon and Intel), Windows (x64 and Arm64), Linux (x64 and Arm64), iPhone on iOS 18 or later, iPad on iPadOS 18 or later, and Android 9 or later.
...
Eligible paths, checked 23 September 2026 against the FAQ and vendor pricing pages: every paid Cursor plan (Pro, Pro+, Ultra), every member of a self-serve Cursor Teams plan, and a linked individual SuperGrok, SuperGrok Plus, SuperGrok Heavy or X Premium+ subscription, plus a one-time trial. Cheapest paid path is Cursor Pro at $20 a month. Cursor Hobby, the free plan, does not include Grok Bot, and neither does SuperGrok Lite. Confirm the live number on the vendor page.

Privacy Mode (Legacy) blocks Grok Bot on every Mac. Do not "fix" it by buying a new laptop. The download sequence is [how to download Grok Bot](/blog/download-grok-bot). Pricing texture is [what Grok Bot actually costs](/blog/grok-bot-cost). Do not spend on an M-series upgrade while the invoice still says Hobby.

There is no Grok Bot-specific spend cap, but the account-level On-demand monthly limit applies. Subscriptions include a weekly usage allowance, then on-demand usage billed from model and token cost. No published dollar figure for the allowance belongs here. Closing the 2019 lid does not freeze that meter.
...
## Author on Intel, pause from the phone, treat Linux as a second desk
...
Install the iPhone app after Porch exists, not as a workaround for a chip you were ashamed of. The split is [Grok Bot on iPhone](/blog/grok-bot-iphone-app). An Android phone is the same pocket client, and the iOS app also runs on iPad; neither replaces this desk.

Linux changed in September 2026. Grok Bot now ships a desktop app for Linux on x64 and Arm64 (.deb, .rpm or AppImage), so dual-booting Ubuntu on the 2019 Intel Mac no longer costs you a client. The computer your bots use is still a separate cloud Linux machine, and installing the Linux app does not make this laptop the worker. If Ubuntu is the daily driver on this hardware, [Grok Bot on Linux](/blog/why-grok-bot-has-no-linux-app) covers the install. Either way, a slow week is fixed by desk habits, not by a faster Intel chip.
...
Soren skipped a written outcome list and still got lucky because send was already on ask. Luck is not a proof. Teach by demonstration (ten minutes max, no mic, browser only, draft skill, not on iPhone) records your local browser. It is the opposite of proving the VM continues without this Intel CPU. A fifth check belongs on invoice day: an identity you know is Hobby should not show a roster. That check fails if you were about to blame Intel for a plan problem.
...
Buying an M-series Mac changes the keyboard you author from. It may make the client UI feel snappier. It does not move the computer, isolate cookies, or add a Bot-specific spend cap. If those were what you wanted from the upgrade, keep the money.
...
|---|---|---|
| Daily driver is Ubuntu on this same Intel hardware | macOS Intel is supported, and so is the Linux desktop app on x64 | [Grok Bot on Linux](/blog/why-grok-bot-has-no-linux-app) |
...
| Android phone or iPad as the planned workstation | Companion apps only. Editing and testing need a desktop | [Supported platforms](/blog/grok-bot-supported-platforms) |
| Invoice is Hobby, or a SuperGrok nobody linked | Chip is irrelevant. Plan excludes Grok Bot, or the link is missing | [Download and eligibility](/blog/download-grok-bot) |
...
Windows users with the wrong architecture have [the Windows setup page](/blog/grok-bot-windows-setup). Do not borrow their Arm64 plot or the Linux SSH plot. Your failure modes are silicon shopping, Rosetta folklore, plan mix-ups, and the lid. Individual accounts and self-serve Teams still have no audit view of Bot actions; Enterprise has audit logs and Action Recording. A new M-series Mac will not grow one. If you cannot accept a shared computer without a product log, do not connect production mail from this desk.
...
Yes. macOS Intel is a full desktop, same authoring jobs as Apple silicon or Windows for create, edit, test, history, and delete. You do not need a newer Mac for week one. You do need this macOS desk before the phone matters. The iPhone app, iOS 18 or later, pauses and resumes, reads run history, and deletes a routine. Editing and testing a routine need desktop. Install the phone client as a stop button after the Intel client already authors jobs. An Android phone or an iPad can be that button too, and you do not need a Linux partition on this Mac.
`,
};
