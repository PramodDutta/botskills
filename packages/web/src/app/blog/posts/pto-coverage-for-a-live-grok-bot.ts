...
PTO coverage for a live grok bot is three moves: pause every send-capable routine on the last desk day, name a cover person with their own eligible account, and recreate only the jobs that must still fire on that person's computer. Routines are not team-level. The cover cannot inherit yours. From the phone app (iPhone or Android) you can approve steps, pause or resume a routine, open its run history, and delete it, but not edit it. Editing and testing wait for a desktop app.
...
Name the cover before the last Friday. Supported clients are macOS (Apple silicon and Intel), Windows (x64 and Arm64), and iPhone on iOS 18 or later ([FAQ](https://docs.x.ai/grok-bot/faq)). There are Linux desktop and Android apps as of September 2026, and the iOS app also runs on iPad (iPadOS 18 or later). The agent computer is a managed Linux VM, not a laptop the intern can SSH into. A Linux-only cover can pause from a phone and recreate a routine on the Linux desktop app. Write three verbs into every send-capable charter: pause before leave, recreate on the cover's seat if the job must run, never share the password. [Inbox Triage](/bots/inbox-triage) is the shape a follow-up desk should already have: drafts wait.
...
| Pause from iPhone if you forgot the desk | Same freeze. No edit, test, or create | Yes as salvage. Then stop |
...
Count the cover's existing routines from a desk before they paste. A fifty-first routine does not schedule. The cap is per bot, not a team pool. The 20 run records are not an audit log ([Grok Bot has no audit view outside Enterprise](/blog/grok-bot-no-audit-log-yet)). Leave your live bot paused, not deleted ([Grok Bot routine did not run](/blog/grok-bot-routine-did-not-run)).
...
They get the persistent cloud computer assigned to you: every bot screen, cookies, sessions, files, and CLI credentials. Hosted MCP sign-in tokens stay with Cursor's backend ([teams and enterprises](https://docs.x.ai/grok-bot/teams-and-enterprises)), and they still act as you because they are signed in as you. Individual accounts and self-serve Teams still have no audit view of Bot actions; Enterprise has audit logs and Action Recording. Changing the password on day nine does not rewind copies from day two. [What Grok Bot actually isolates](/blog/grok-bot-shared-computer-security) is the architecture. [The Hobby exclusion page](/blog/grok-bot-hobby-and-pro-exclusion) is why Cursor Hobby, the free plan, is not a door. Neither page is permission to lend them yours.
...
Friday 21 August 2026, 16:40. Maren still has Follow-up Clerk on Cursor Pro at $20 a month, the cheapest paid path that includes Grok Bot, checked 23 September 2026 ([Cursor pricing](https://cursor.com/pricing)). The bot owns four weekday routines, including a 07:00 fire that can send. Morning Brief sits on a second card in the shape of [Chief of Staff Briefing](/bots/chief-of-staff-briefing): read only. Standup Pack sits on a third. All three share one computer and one Gmail cookie.
...
Documented mobile scope, from the [mobile page](https://docs.x.ai/grok-bot/mobile): iPhone on iOS 18 or later can pause and resume a routine. Editing and testing a routine still need the desktop app; the phone can now show run history and delete a routine. Teach by demonstration is unavailable on iPhone. You will not rewrite Follow-up Clerk, fire a test, or create Ned's replacement routine from the jetway.

If Friday's desk pause did not happen, the airport tap is salvage. Open Grok Bot, not grok.com chat, not Cursor, not Mail. Pause each send-capable routine you can reach. Stay until frozen. Write the names and clock time in Notes. Do not resume when boarding group C is called. Ned cannot create the replacement brief from a pocket either. If the only machine in his bag is an iPad, it runs the iOS app (iPadOS 18 or later) with the phone's limits, so it cannot create the routine either; a Linux laptop runs the desktop app as of September 2026 and can ([supported platforms](/blog/grok-bot-supported-platforms)). Staff the last Friday as a desk hour for both people.
...
## Pay for the cover's Teams seat instead of lending your Cursor login

Ned on Hobby cannot open Grok Bot. Cursor Hobby, the free plan, does not include it; every paid Cursor plan does, from Cursor Pro at $20. An individual SuperGrok subscription can be linked instead. Eligible paths, checked 23 September 2026: Cursor Pro, Pro+ and Ultra, Cursor Teams, a linked individual SuperGrok, SuperGrok Plus, SuperGrok Heavy or X Premium+ subscription, plus a one-time trial ([FAQ](https://docs.x.ai/grok-bot/faq), [Cursor pricing](https://cursor.com/pricing)). Confirm the live invoice the morning you pay.

Every seat on a self-serve Cursor Teams plan includes Grok Bot as a per-person stamp ([Grok Bot on Cursor Teams Standard](/blog/grok-bot-teams-standard)). Cursor Pro at $20 is the cheapest paid individual path. Pro+ at $60 and Ultra at $200 include it with more weekly usage. A Teams Premium seat is the same Bot product on a richer Cursor SKU. None of those SKUs mint a shared bot. Each seat gets its own cloud computer.
...
|---|---|---|
| Cursor Hobby | No | Buy a Teams seat or Cursor Pro, or a qualifying trial. Or pause |
| SuperGrok (individual) | Yes, by linking | Link it from the Grok Bot plan screen; it grants usage and is not a Cursor plan |
| Cursor Pro at $20, Pro+ at $60, Ultra at $200, linked SuperGrok Plus or Heavy | Yes, if the live invoice still says so | Recreate must-run jobs on their bot. Never on Maren's login |
| A seat on a self-serve Cursor Teams plan, Standard or Premium | Yes, per seat | Same recreate rule. Teams is not a shared bot |
| No seat, no trial, Linux-only bag | No. The Linux desktop app still needs a seat. iPhone pause only if iOS 18+ | Pause Maren's send-capable clocks. Do not DM the password |
...
Before leave (desktop app, not iPhone):
...
- Phone app: approve steps, pause or resume a routine, read run history, delete a routine. Editing and testing wait for a desk.

On return:
- Resume only from a desktop after reading /workspace and Sent.
- Do not treat a missing history row as proof nothing happened. Individual accounts and self-serve Teams still have no audit view of Bot actions; Enterprise has audit logs and Action Recording. Check the destination.
...
Watching does not split the home directory. An unlocked office Mac with her session is her computer. A login from his house is her computer. Two people on one user account are still one cloud computer and one cookie jar. There is no audit view outside Enterprise, so "I will look over his shoulder" is not a log. If he can resume, he can send. If he can tidy the sidebar, he can delete routines. If he can only watch, he needs exported packs and a pause that already held.
...
Delete feels like a clean OOO. Deleting a bot removes its profile, conversation, and routines. It does not retract mail, sign out of Gmail, empty /workspace, or clear the cookie jar. You also destroy the twenty most recent run records per routine. The phone app can delete too, so a pocket is no safer. Do not ask Ned to delete your bots as a favor. [How to delete a Grok Bot without leaving logins behind](/blog/delete-a-grok-bot-safely) is teardown for a planned retirement. A leave is not a retirement.

Enterprise organization admins can now Terminate a member's computer; it still keeps the durable disk, and the next session starts a fresh computer on it. Do not staff PTO as if Terminate were a leave button. Do not staff PTO around the team-level ceiling on local execution either; it caps the member's own machine, not routines on the cloud computer ([teams and enterprises](https://docs.x.ai/grok-bot/teams-and-enterprises)). Pause is the verb that exists. Leave Follow-up Clerk in the sidebar, paused, charter intact, so resume on 30 August is a desk decision rather than a rebuild.
...
It also stops applying when there is no live send-capable routine and no must-run clock. A draft-only fleet can sit paused without a cover seat. A missed Monday because someone deleted the owner bot last week is [a routine that did not run](/blog/grok-bot-routine-did-not-run). Companies that need a team on-call calendar and a dedicated VM per bot are describing a product Grok Bot does not ship today, and a vendor audit log ships only on Enterprise. Do not staff an eight-day leave as if it were filled.
...
No. The login is the computer, not a coverage shift. All bots share one persistent cloud computer assigned to your user, not to a bot. Cookies, sessions, files, and CLI credentials go with whoever can sign in. Screens are not security boundaries. Individual accounts and self-serve Teams still have no audit view of Bot actions; Enterprise has audit logs and Action Recording. Pause send-capable routines before you leave. If a job must fire, recreate it on the cover person's own eligible seat. Changing the password on day nine does not rewind copies they made on day two.

### If I pause everything, how does the cover person still run Monday standups?

They run Monday on their computer, not on yours. A routine belongs to one bot. Nothing is team-level. Buy them an eligible seat (a seat on a self-serve Cursor Teams plan is one path, checked 23 September 2026) or a qualifying trial, then recreate the standup on a bot they own, from a desktop. The phone app cannot create that routine; it needs the desktop app. If you will not buy a seat, pause and skip the bot standup. Email a human checklist. Do not paste the founder password so their Monday pack can fire.

### Can I handle PTO coverage from iPhone at the airport?

You can pause. That is the documented pocket pair, with resume, and resume is the wrong leave verb. Editing and testing a routine still need the desktop app; the phone can now show run history and delete a routine. You will not create the cover's replacement routine from the jetway. If Friday's desk pause did not happen, pause every send-capable routine you can reach, stay until frozen, and write the names in Notes. Staff the last desk day for both people. An Android-only or iPad-only bag cannot recreate coverage until someone borrows a desktop.
