...
export const post: BlogPost = {
  title: 'Update Forecast Notes, Never Edit the Forecast Figure',
  description:
    'Use Grok Bot to assemble sourced forecast notes and exception queues while every amount, close date, probability, and category stays human controlled.',
  date: '2026-08-29',
  category: 'Tutorial',
  content: `
# Update Forecast Notes, Never Edit the Forecast Figure

Mara has forty-two opportunities to review before Thursday's forecast call. The figure in the dashboard says one thing, the latest proposals say another, and six next steps expired last week. She does not need a machine to choose a better number. She needs a compact record of why each number deserves a human review.

That is the useful job for **grok bot forecast notes**: read approved evidence, attach it to the right opportunity, name contradictions, and draft a private note. The bot never changes amount, close date, stage, probability, forecast category, owner, or customer communication. The forecast figure remains a decision made through the governed sales process.

This tutorial builds that narrow routine, follows Mara through one failed run, and shows how to prove the boundary with fixtures that try to make the bot write.

## Treat the forecast figure as a decision and the note as evidence
...
## Resolve one opportunity before reading any narrative
...
## Register which source owns each field before comparing values
...
## Preserve unknown and conflicting states instead of completing the story
...
## Write a charter that can finish without CRM write access
...
## Produce a note whose shape exposes rather than hides uncertainty
...
## Walk Mara through one deal from contradiction to reviewed note
...
## Diagnose the failed run that silently selected the newest proposal
...
## Route each exception to the person who owns the source
...
## Schedule the routine around evidence freshness rather than urgency
...
## Test the no-edit boundary with hostile fixtures
...
## Verify every run with evidence that can fail
...
## Answer the manager who wants automatic cleanup of obvious fields
...
## Keep notes private until a named reviewer accepts them
...
## Stop this workflow where forecasting policy begins
