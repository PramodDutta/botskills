...
export default async function IntegrationPage({ params }: Props) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://botskills.sh' },
      {
        ...
        position: 2,
        name: 'Integrations',
        item: 'https://botskills.sh/integrations',
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: hub.name,
        item: `https://botskills.sh/integrations/${hub.id}`,
      },
    ],
  };
  ...
  return (
    <main className="wrap detail">
      ...
      />
      <Link href="/integrations" className="ds">
        &larr; All integrations
      </Link>
      <h1>Grok Bot and {hub.name}</h1>
      <p className="sub">
        <IntegrationIcon id={hub.id} /> {hub.bots.length} {hub.bots.length === 1 ? 'bot' : 'bots'}{' '}
        in the directory connect to {hub.name}
        {hub.posts.length > 0
          ? `, and ${hub.posts.length} ${hub.posts.length === 1 ? 'guide covers' : 'guides cover'} setting it up`
          : ''}
        . Each bot declares the one action it never takes without you, which for a connected tool is
        usually the write, the send or the payment.
      </p>

      <h2>Bots that connect to {hub.name}</h2>
      {hub.bots.map((b) => (
        <p key={b.slug}>
          <Link href={`/bots/${b.slug}`} className="nm">
            {b.name}
          </Link>
          <br />
